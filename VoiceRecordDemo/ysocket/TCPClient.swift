    ...
    */
    func read(expectlen:Int) -> [UInt8]? {
        if let fd:Int32 = self.fd {
            ...
            let readLen: Int32 = c_ytcpsocket_pull(fd: fd, buff: &buff, len: Int32(expectlen))
            if readLen <= 0 {
                return nil
            }
    ...
    func connectServer(timeout t:Int) {
        ...
        if rs > 0 {
            ...
            dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0).asynchronously() { [weak self] in
                while self != nil {
                    //读取数据
